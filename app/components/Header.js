...
export default function Header({ transparentAtTop = false }) {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-[100] flex justify-center px-4 pt-4">
      ...
      <div
        className="kt-nav-in grid w-full max-w-5xl grid-cols-[1fr_auto_1fr] items-center gap-4 border px-6 py-3 backdrop-blur-xl backdrop-saturate-150 transition-[background,border-color,box-shadow,color] duration-500"
        style={{
      ...
      >
        ...
        <div className="flex items-center justify-end gap-[1.1rem]">
          <div className="hidden items-center gap-[1.1rem] min-[880px]:flex">
            <Link
              href="/reservation"
              className={`${ctaBase} px-[1.1rem] py-2 text-[0.78rem] tracking-[0.04em]`}
            >
              Book Now
              <CtaArrow />
            </Link>
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center border-0 bg-transparent p-0 transition-colors duration-200 hover:bg-white/10 min-[880px]:hidden"
      ...
      <div
        className={`absolute inset-x-4 top-[4.75rem] z-[99] flex flex-col overflow-hidden border border-white/30 bg-white/80 px-6 text-ink shadow-[0_20px_50px_-12px_rgba(22,24,31,0.35)] backdrop-blur-xl backdrop-saturate-150 transition-all duration-300 min-[880px]:hidden ${
